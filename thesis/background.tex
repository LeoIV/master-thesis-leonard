...
\paragraph{StyleGAN}
\begin{itemize}
    \item \citet{karras2019style} is a GAN approach addressing feature disentanglement in the Generator
\end{itemize}
...
\textbf{DRAFT!}
\citet{pidhorskyi2020adversarial} describe a model combining \acp{VAE} and \acp{GAN} that particularly addresses feature disentanglement.
They combine \acp{VAE} and StyleGAN~\citep{karras2019style}.
In particular, they do not use a pixel-wise loss.
By splitting up both, the Generator and the Discriminator into two networks, they map the latent distribution $\bm{z}$ into another latent distribution $\bm{w}$.
By introducing noise, they are able to traverse the $\bm{w}$-space, however I am not sure what form this space has.
Using this architecture, they are able to use an $l^2$ loss on the latent space, mitigating the problem of the pixelwise loss on the reconstructions.
Forcing reconstruction to map the data distribution is done by using the discriminator.

There are some open questions w.r.t. this paper:
\begin{itemize}
    \item Is this approach biologically plausible? It seems as if encoder and decoder are feedforward as opposed to \ac{VLAE} and \ac{LVAE}.
    \item How exactly are samples generated. The interpolation between numbers is clear but how do you sample from $\bm{w}$-space?
\end{itemize}

...
Reinforcement learning is a quite different paradigm from supervised and unsupervised learning.
Here, the aim is to train an \textit{agent} to achieve a goal.

An example are rats in a maze.
...
A value of $\beta > 1$ puts more emphasis on the KL-term than the original \ac{VAE} loss function ($\beta = 1$).
...
\acp{ALAE} are not based on variational inference but on the \acp{GAN} training objective.
...
For an in-depth discussion on representation learning with autoencoders, the reader is referred to \citet{tschannen2018recent}.
...
Then, \textit{shape}, $x$\textit{-position}, and $y$\textit{-position} are the \say{factors of variation}~\citep{Shu2020Weakly,higgins2018towards}, say $S_1$, $S_2$, and $S_3$ with $S_i\in \mathcal{S}^n$~\citep{Shu2020Weakly}.
...
Furthermore, it is often desired that the sampling from the latent space leads to the data distribution~\citep{karras2019style}.
...
Consistency means that if only one factor of variation is varied (e.g., the factor that attributes to an object's size), generated items only change for this very factor but not to other factors (i.e., only the object's size is changing but not its shape).
...
For example, if factor $i$ again encodes an object's size, the same value of factor $i$ should always encode the same size, irrespective of the choice of the other factors~\citep{Shu2020Weakly}.
...
The missing square in the upper left is the combination \say{old and high voice level} that is not present in the dataset.
...
If the latent space is disentangled, there should be one or multiple almost invariant dimensions in the generated data that accounts for to the fixed factor of variation.

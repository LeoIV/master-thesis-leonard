...
% How many regions does the visual cortex have? Introduce that first
...
% TODO: Two other pathways transport signals from the retina to other brain regions but are not concerned with object recognition but controlling movements and pupillary reflexes~\citep[p. 559]{mack2013principles}.
% TODO: Introduce receptive field
...
In contrast, higher regions detect higher-level features~\citep{eickenberg2017seeing}:
...
% TODO: gucken ob visual attention ein feststehender Begriff ist
% TODO: Zitat umschreiben
Furthermore, the \ac{V4} is assumed to play a role in \say{visual attention}~\citep{roe2012toward}, i.e.,~to \say{enhance neuronal firing to relevant stimuli in V4 and [to] suppress responses to distractor stimuli}~\citep{roe2012toward}.
...
% TODO Gucken ob ich das zitieren muss
...
% TODO: Was ist Gabor wavelet und Neuron Excitation?
...
One option is to not pad the image at all and to start and end the convolution at indices such that the sum in Equation~\ref{eq:conv_boiled_down} is never applied to invalid image indices~\citep[p. 350]{Goodfellow-et-al-2016}.
...
% TODO: Hier muss das Zitat unbedingt weg
In the visual cortex, only hierarchical representations can be observed~\citep{rodriguez2015hierarchical}.
% TODO: Brücke zum Representation learning, oder den Satz darüber weg
...
% TODO: Woher kommt das? Beschreibe den Anwendungsfall hier
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption[\ac{VLAE} structure]{\ac{LVAE} structure, adapted from \citet{sonderby2016ladder}. Solid arrows indicate feed-forward connections, dashed arrows indicate weight sharing. Diamonds indicate stochastic variables, circles deterministic variables.}
    \label{fig:lvae}
\end{figure}
% TODO: Beschreib top-down priming hier
% TODO: Warum ist das ein Problem?
...
% TODO: Verweis auf Perception von Menschen hier
...
Furthermore, the additional top-down pass is more biologically plausible compared to a pure feed-forward network~\citep{rodriguez2015hierarchical}.

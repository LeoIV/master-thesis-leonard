...
Section~\label{subsec:types-of-learning} used the example of a \say{cat-and-dog-classifier} to introduce supervised learning.
...
\textbf{TODO: Posterior Collapse (Dai et al. 2019), Spurious global optima (van den Oord et al. 2018), prior/posterior mismatch (Tolshkin et al. 2018, Dai et al. 2019)}
...
Instead, they use a deterministic multi-layer network in the encoder (see Figure~\ref{fig:vlae}) and pass information from intermediate layers to the stochastic embedding layers.
\begin{align}
...
Unlike the \ac{LVAE}, the \ac{VLAE} is a pure feedforward network.

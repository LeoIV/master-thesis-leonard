...
Here, the aim is to train an \textit{agent} to show some meaningful behavior.

An example is rats in a maze.
A piece of cheese is located in one corner of the maze.
The rat has multiple possible actions, going forward, backward, left, or right.
In this setting, the rat is the agent.
Its reward is cheese, and the rat will maximize its reward.
By reinforcing good sequences of actions, the rat eventually shows a reasonable behavior.

Reinforcement learning algorithms resemble such settings by modeling environment, action space, and reward~\citep[pp. 1, ff.]{sutton2018reinforcement}.
...
For a model trained on CelebA\footnote{See Section~\ref{celeba_dataset}.} it is possible to add sunglasses to a randomly generated face by adding the mean latent vector of images containing sunglasses~\citep{radford2016deep,hou2017deep}.
...
A disentangled representation is one

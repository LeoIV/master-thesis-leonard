...
\begin{figure}
    ...
    \caption{Histogram of mean $z$-values for different layers and dimensions of \ac{VLAE}-\ac{GAN} (blue), the result of the \ac{KDE} (green), and a standard normal distribution (orange). Additional plots can be found in Appendix~\ref{sec:appendix_pixel_wise_statistics}.}
...
Subsequently, for each dimension and each layer (in case of \ac{VLAE} or \ac{VLAE}-\ac{GAN}), \ac{KDE} was performed to estimate the \ac{PDF} of the latent space.
...
Figure~\ref{fig:vlae_gan_kde} shows the estimated \ac{KDE} for \ac{VLAE}-\ac{GAN}, plots for the other models can be found in Appendix~\ref{sec:appendix_pixel_wise_statistics}.
...
\begin{table}
    \begin{tabular}{lrrrr}
        ...
        \midrule
        \ac{VAE}           & $2.4\cdot 10^{-34}$ & 0.0                  & $4.1\cdot 10^{-227}$ & $1.4\cdot 10^{-90}$  \\
        \ac{VLAE}          & 0.277               & $2.9\cdot 10^{-117}$ & $3.7\cdot 10^{-12}$  & 0.004                \\
        \ac{VAE}-\ac{GAN}  & $1.4\cdot 10^{-79}$ & 0.0                  & $1.0\cdot 10^{-28}$  & $5.0\cdot 10^{-23}$  \\
        \ac{VLAE}-\ac{GAN} & 0.055               & $4.7\cdot 10^{-13}$  & $5.1\cdot 10^{-8}$   & $1.1\cdot 10^{-14}$  \\
        \bottomrule
    \end{tabular}
    \caption{$p$-values of an unpaired Student's $t$-test. Each cell tests the hypothesis that the respective moments for the respective model are equal to the values for the \textsc{MNIST} test images. For each cell, the sample size was $2\cdot 10,000$.}
    \label{tab:vae-vlae-mnist}
\end{table}
Table~\ref{tab:vae-vlae-mnist} shows the results of an unpaired Student's $t$-test for the samples of moments of the pixel distributions.
...
\textit{The reason for this probably is that digits appear more similar for some classes than for others.
The model tries to minimize the reconstruction loss while keeping the \ac{KL-divergence} minimal and therefore uses more of the latent space for digit classes with more dissimilar digits.
This under-representation of object classes with more similar objects should be considered when working with \acp{VAE} as it}

\textbf{This makes no sense because the less frequent-numbers are different for different models. See if the more common digits are placed in latent space regions with probability density. If so, discuss the problems with this.}

...
Figure~\ref{fig:classification_layer1_kernels} shows all 288 ($96 \cdot 3$) convolutional kernels of AlexNet Classifier (see Section~\ref{subsubsec:alexnet_classifier}).
...
\begin{figure}
    ...
    \begin{subfigure}{.95\textwidth}
        \centering
        % include first image
        \includegraphics[height=.95\textheight]{images/sparseness/encoder_fm1_fms.png}
...
\begin{figure}
    ...
    \begin{subfigure}{.95\textwidth}
        ...
        \includegraphics[height=.95\textheight]{images/sparseness/encoder_fm2_fms.png}
...
\begin{figure}
    ...
    \begin{subfigure}{.95\textwidth}
        \centering
        % include second image
        \includegraphics[height=.85\textheight]{images/sparseness/encoder_fm3_fms.png}
        \caption{Feature map activities - small model}
    \end{subfigure}
    \caption{Feature map activities for the different models.
    Each boxplot corresponds to the distribution of one feature map after the convolution.
    The activities are scaled by the mean weights of the following convolutional kernel.
    }
...
Figure~\ref{fig:fm_activities_sparseness} shows the activities of the feature maps in the different models, scaled by the weights of the following convolutional kernel.
The bias terms are not considered:
If the variations in the feature maps are negligible, the feature map itself acts as a bias term for the network.
Therefore, only the scaled feature map activity variations are considered.
The network employs sparseness - only few feature maps seem to heavily influence the network.
...
Yet, this analysis is only approximative since averaging over the weights in the convolutional kernel leads to information loss.
Future work should further investigate if the assumption that \acp{VAE} do not employ sparseness similarly to the brain holds.
...
To analyze how \acp{VAE} behave for categorical factors of variation (RQ3, see Table~\ref{tbl:research_questions}), a \ac{VAE} with input size $64\times 64$ (the size of dSprites images) and a ten-dimensional latent space was trained on dsprites with different reconstruction term weights: 10,000, 7,500, 6,250, 5,000, and 3,750 (see Section~\ref{subsubsec:vae_models})\footnote{It was first empirically validated that a reconstruction term weight of 10,000 leads to good reconstruction. The other values were chosen by reducing the reconstruction loss term by 1,250 for each model with a larger initial difference (10,000 vs. 7,500).}.
...
Therefore, it should be possible to almost uniquely identify digit \say{1} by just considering the stroke length or the digit width.
...
Figure~\ref{subfig:vlae_mnist_latent_space_z_1_slant} shows the embedding layer $\bm{z}_1$ colored by digit slant.
...
The latent space embeddings for the dSprites dataset allow a detailed comparison of \ac{VAE} and \ac{VLAE}.
...
Averaging over latent space positions if images with high thickness is less precise since only few images contribute to the mean value on the \say{high-thickness-bins}.
...
Figure~\ref{fig:celeba_latent_space_traversal} shows the latent space exploration of CelebA-\ac{VLAE}-\ac{GAN} model (see Section~\ref{subsubsec:vlae_gan_models}).
...
The behavior does not change for the other models (7,500-, 6,250-, 5000-, 3750-\ac{VAE}) or different shapes.
...
There is no reason to assume that \acp{VAE} are a better model of the ventral than of the dorsal stream.
Moreover, \acp{VAE} seem to learn positional factors of variation differently from non-positional ones.
For factors of variation such as \say{hair color} or \say{gender} (for the CelebA dataset), a simple linear traversal in the latent space is sufficient to interpolate between these factors.
...
For dSprites-\ac{VLAE} model, a similar analysis allows to identify which layers encode which factor of variation\footnote{The plots for the corresponding dSprites-\ac{VLAE}-\ac{GAN} model can be found in Appendix~\ref{sec:appendix_plots_latent_space_traversals}.}.
\begin{figure}
    \centering
    \begin{subfigure}{.48\textwidth}
        \centering
...
The first and second layers of dSprites-\ac{VLAE} jointly learn orientation, similarly to dSprites-\ac{VAE}.
...
Figure~\ref{fig:mean_generated_vs_true} leads to the assumption that all models learn the pixel distributions to some extent.
...
(2) None of the models captures the true pixel distribution (rejection of $H_0$: \say{The distributions are the same.}).
...
Other than the models used in this thesis, the human eye receives a stream of visual stimuli.
...
The \ac{VLAE} and \ac{VLAE}-\ac{GAN} used in the course of this thesis use three latent spaces compared to one latent space for a regular \ac{VAE}(-\ac{GAN}).

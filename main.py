...
def main(args: List[str]):
    ...
    if args.configuration == 'simple_classifier':
        ...
        if 'gan' not in args.configuration:
            model = VAE(input_dim=input_dim, encoder_conv_filters=[32, 64, 128, 256, 512],
                        encoder_conv_kernel_size=[4, 4, 4, 4, 4], encoder_conv_strides=[2, 2, 2, 2, 1],
                        decoder_conv_t_filters=[512, 256, 128, 3 if args.rgb else 1],
            ...
            model = VAEGAN(input_dim=input_dim, encoder_conv_filters=[32, 64, 128, 256, 512],
                           encoder_conv_kernel_size=[4, 4, 4, 4, 4], encoder_conv_strides=[2, 2, 2, 2, 1],
                           decoder_conv_t_filters=[512, 256, 128, 3 if args.rgb else 1],
                           decoder_conv_t_kernel_size=[4, 4, 4, 4, 4],
                           decoder_conv_t_strides=[2, 2, 2, 2, 1], log_dir=args.logdir, z_dims=args.z_dims,
        ...
        if 'gan' not in args.configuration:
            model = VAE(input_dim=input_dim, encoder_conv_filters=[32, 64, 128, 256, 512],
                        encoder_conv_kernel_size=[4, 4, 4, 4, 4], encoder_conv_strides=[2, 2, 2, 2, 2],
                        decoder_conv_t_filters=[512, 256, 128, 64, 3 if args.rgb else 1],
                        decoder_conv_t_kernel_size=[4, 4, 4, 4, 4],
                        decoder_conv_t_strides=[2, 2, 2, 2, 2], log_dir=args.logdir, z_dims=args.z_dims,
        ...
        if 'gan' not in args.configuration:
            model = VLAE(input_dim=input_dim, log_dir=args.logdir,
                         ...
                         inf1_kernels_strides_featuremaps=[(3, 2, 128)],
                         ladder0_kernels_strides_featuremaps=[(5, 2, 64)],
                         ladder1_kernels_strides_featuremaps=[(5, 2, 128)],
                         ladder2_kernels_strides_featuremaps=[(3, 1, 256), (3, 1, 512)],
                         gen2_num_units=[1024, 1024],
                         gen1_num_units=[1024, 1024],
                         gen0_kernels_strides_featuremaps=[(4, 2, 512), (4, 2, 256), (4, 1, input_dim[-1])],
                         kernel_visualization_layer=args.kernel_visualization_layer, num_samples=args.num_samples,
